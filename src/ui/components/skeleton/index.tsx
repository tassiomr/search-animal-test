...
const generateSkeletons = (limit: number, size: SkeletonSizes) => {
  return Array.from({ length: limit }, (_, index) => (
    <div
      ...
      className={`skeleton-nested skeleton-loading ${size}`}
    ></div>
  ));
};
...
export const Skeleton: React.FC<Props> = ({
  ...
  if (limit > 1) {
    return (
      <div data-testid="skeleton-build-container">
        {generateSkeletons(limit, size)}
      </div>
    );
  }

  return (
    <div data-testid={testId} className={`skeleton-loading ${size}`}></div>
  );
};
